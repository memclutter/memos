...
- [ ] 1. Scaffold `src/memos/doctor.py` and wire a `doctor` subcommand in `cli.py`
  (handler returns `0`, no checks yet) — establishes the plumbing.
- [ ] 2. Implement `check_shims(root) -> list[str]`: compare `compute_shims(root)`
  to disk (missing / byte-mismatch) and detect stale shim folders per `TOOLS`.
- [ ] 3. Implement `check_rules_index(root) -> list[str]`: parse `AGENTS.md` for
  `rules/*.md` links; report unindexed rules and links to missing files.
- [ ] 4. Fill in the `doctor` handler: run both checks, print a per-check summary
  (problems or "ok"), collect **all** problems, exit `1` if any else `0`.
- [ ] 5. Add `tests/test_doctor.py`: `check_shims` and `check_rules_index` cases
  over `tmp_path` (clean → none; each violation → reported) + a smoke test that
  `doctor` on the real repo returns `0`.
- [ ] 6. Add `.pre-commit-config.yaml` — `repo: local` hook `memos-doctor`
...
- [ ] 7. Create `.github/workflows/ci.yml` (push + pull_request): `checkout` →
  ...
  from the repo root.
- [ ] 8. Green check: `uv run memos doctor` exits `0`, and ruff/mypy/pytest pass.
