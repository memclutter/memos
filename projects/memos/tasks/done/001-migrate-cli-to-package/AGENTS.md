...
- [x] 1. `git rm` the old single-file `scripts/memos` (frees the name so
  `scripts/memos/` can become the package directory).
- [x] 2. Add the two `pyproject.toml` files: root virtual workspace
...
- [x] 3. Scaffold `scripts/memos/src/memos/`: `__init__.py`, `__main__.py`
...
- [x] 4. Port `shimify` into `shims.py` as `compute_shims(root) -> {path: content}`
  (pure) + `write_shims(root)` (I/O), keeping behaviour identical (frontmatter
  split, skill loading, shim text).
- [x] 5. Wire the `shimify` subcommand in `cli.py` to call `write_shims` and print
  the same summary line.
- [x] 6. Verify byte-identical output: `uv run memos shimify` leaves **no git
  diff** on the shim dirs.  ⚠️ gate — headline acceptance criterion; ask owner if
  any diff appears.
- [x] 7. (folded into step 1 — the old file is removed up front.)
- [x] 8. Rename `uv run scripts/memos` → `uv run memos` and fix "where the CLI
...
- [x] 9. Add `tests/test_shims.py`: stability (`compute_shims(root)` == bytes on
  disk) and round-trip in `tmp_path` (mutated shim detected as different).
- [x] 10. `uv run ruff check`, `uv run mypy`, and `uv run pytest` all pass.
