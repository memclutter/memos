...
- [x] 1. Add a mock-NocoDB test helper: an `httptest.Server` that routes on
...
- [x] 2. Unit-test `internal/api` (`nocodb_test.go`), table-driven: method/path,
...
- [x] 3. Unit-test `internal/storage` (`migrations_test.go`):
...
- [x] 4. Unit-test `internal/migration` (`operations_test.go` + `executor_test.go`):
...
- [x] 5. Add `.golangci.yml` (govet, staticcheck, errcheck, gofmt/goimports,
  ineffassign, unused) and make `go test ./...` + lint green locally.
- [x] 6. Add `.github/workflows/ci.yml` with a `lint-unit` job (setup-go, fmt
  check, `golangci-lint run`, `go test ./...`) on push/PR.
- [x] 7. Add `.pre-commit-config.yaml` (fmt/goimports, golangci-lint, fast
